...
internal sealed class AddProductCommandHandler : ICommandHandler<AddProductCommand, long>
{
    ...
    {
        var product = Product.Create(new ProductName(request.name), new ProductDescription(request.description), new Money(request.priceAmount, Currency.Create(request.priceCurrency)), request.quantity, request.categoryId, _dateTimeProvider.UtcNow);
